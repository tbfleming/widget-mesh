    ...
    return {
        ...
        addThreeMesh: function (threeMesh, attrs) {
            let mesh = {
                ...
                threeMesh: threeMesh,
                types: ['3d'],
                edges: new THREE.EdgesHelper(threeMesh, 0x000000),
            };
            ...
            chilipeppr.publish('/com-chilipeppr-widget-3dviewer/sceneadd', mesh.edges);
        ...
        // Get a new typed array with transformed vertexes
        // public: other widgets may call this directly
        getTransformedVertexes: function (threeMesh) {
            let geometry = threeMesh.geometry;
            if (!(geometry instanceof THREE.BufferGeometry))
                geometry = new THREE.BufferGeometry().fromGeometry(geometry);
            let v = geometry.getAttribute('position').array.slice();
            let m = threeMesh.matrix.elements;
            for (let i = 0; i < v.length; i += 3) {
                let x = v[i + 0];
                let y = v[i + 1];
                let z = v[i + 2];
                v[i + 0] = m[0] * x + m[4] * y + m[8] * z + m[12];
        ...
        // Get paths from vertexes. vertexes must be a typed array. Returns [] if the mesh isn't flat.
        // public: other widgets may call this directly
        getPathsFromVertexes: function (vertexes, scale) {
            if (vertexes.length < 9)
                return [];

            let z = vertexes[2];
            for (let i = 0; i < vertexes.length; i += 9)
                if (vertexes[i + 2] !== z || vertexes[i + 5] !== z || vertexes[i + 8] !== z)
                    return [];

            // Clipper slows to a crawl in JoinCommonEdges() when processing some meshes.
            ...
            sorted.sort((a, b) => {
                if (vertexes[a + 0] === vertexes[b + 0])
                    return vertexes[a + 1] - vertexes[b + 1];
                else
                    return vertexes[a + 0] - vertexes[b + 0];
            });

            // Map each vertex to a unique vertex
            let vertexToUnique = new Int32Array(sorted.length);
            let unique = 0;
            for (let i = 0; i < sorted.length; ++i) {
                if (!i ||
                    vertexes[sorted[i] + 0] !== vertexes[unique + 0] ||
                    vertexes[sorted[i] + 1] !== vertexes[unique + 1]) {
                    unique = sorted[i];
                }
                vertexToUnique[sorted[i] / 3] = unique;
            }
            ...
            for (let i = 0; i < vertexes.length; i += 9) {
                ...
                let e1 = addEdge(i + 0, i + 3);
                let e2 = addEdge(i + 3, i + 6);
                let e3 = addEdge(i + 6, i + 0);
                e1.prev = e3;
                e2.prev = e1;
                e3.prev = e2;
            }
            ...
            // set firstDuplicateIndex
            unique = 0;
            for (let i = 0; i < edges.length; ++i) {
                let edge = edges[i];
                if (edge.v1 != edges[unique].v1 || edge.v2 != edges[unique].v2)
                    unique = i;
                edge.firstDuplicateIndex = unique;
            }
            ...
            for (let i = 0; i < edges.length; ++i) {
                ...
                while (stack.length) {
                    let edge = stack.pop();
                    let merged = false;
                    for (let j = edge.firstDuplicateIndex; j < edges.length; ++j) {
                        if (edges[j].v1 != edge.v1 || edges[j].v2 != edge.v2)
                            break;
                        if (!edges[j].used && edges[j].reversed != edge.reversed) {
                            merged = true;
                            edge = edges[j];
                            while (!edge.used) {
                                edge.used = true;
                    ...
                    if (!merged) {
                        if (edge.reversed)
                            path.push({ X: vertexes[edge.v2 + 0] * scale, Y: vertexes[edge.v2 + 1] * scale });
                        else
                            path.push({ X: vertexes[edge.v1 + 0] * scale, Y: vertexes[edge.v1 + 1] * scale });
                    }
                }
                paths.push(path);
            }

            return ClipperLib.Clipper.SimplifyPolygons(paths, ClipperLib.PolyFillType.pftPositive);
        }, // getPathsFromVertexes
        ...
        // public: other widgets may call this directly
        polyTreeToTriangles: function (polyTree, z, scale, result) {
        ...
        // public: other widgets may call this directly
        offsetPaths: function (paths, delta) {
                ...
                this.meshes.map(mesh =>
                    h('tr', {
                        ...
                        h('td', h('input', {
                            ...
                            onclick: e => { mesh.threeMesh.visible = mesh.edges.visible = e.target.checked; this.wakeanimate(); }
                        })),
                        h('td',
                            {
